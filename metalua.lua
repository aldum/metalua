...
if not pcall(require, 'checks') then
  function package.preload.checks()
...
function M.cmdline_parser(...)
  ...
  for i, short in pairs(opts) do
    ...
    if chunk_options[long] then
      table.insert(cfg.chunks, { tag = long, optarg[i] })
    else
      cfg[long] = optarg[i] or true
    end
  end
